...
const validatePromotion = (promotion) => {
  ...
  if ((promotion.start_date && now < promotion.start_date) || (promotion.end_date && now > promotion.end_date)) {
    throw new Error('Promotion not in valid time')
  }

  // flash sale check
  if (promotion.type === 'flash_sale' && promotion.quantity_limit && promotion.sold >= promotion.quantity_limit) {
...
// list assignment promotion-product
exports.listAssignments = async (query) => {
  const filter = {}

  if (query.promotion_id) {
    filter.promotion_id = query.promotion_id
  }
  ...
  return await ProductPromotion.find(filter)
...
exports.getBestPromotionByVariant = async (variant_id) => {
  ...
  const promotions = links.map((l) => l.promotion_id).filter(Boolean)
